class Personne
  ...
  def info
    ...
    if @en_vie == true
    	return "#{@nom} a #{@points_de_vie} PV"
    # - Renvoie le nom et "vaincu" si la personne a été vaincue
    	else
    		return "#{@nom} a tété vaincu"
    end
  end
  ...
  def subit_attaque(degats_recus)
    ...
    # - Affiche ce qu'il s'est passé
    	puts "#{@nom} a reçu #{degats_recus} points de dégats"
...
class Joueur < Personne
  ...
  def degats
    # A faire:
    # - Calculer les dégats
    	degats = rand(15..25) + @degats_bonus
    # - Affiche ce qu'il s'est passé
  end
  ...
  def soin
    ...
    # - Affiche ce qu'il s'est passé
      if @points_de_vie >= 100
      	puts "Vous avez récupéré tous vos points de vie"
      	@points_de_vie = 100
      else
      	puts "Vous avez été soigné de #{soin} PV"
      	puts "Vous possedez maintenant #{@points_de_vie} PV"
      end  		
  end
  ...
  def ameliorer_degats
    ...
    # - Affiche ce qu'il s'est passé
    puts "Jean michel inflige maintenant #{degats_bonus} de dégats en plus !"
  end
end
...
class Jeu
  ...
  def self.est_fini(joueur, monde)
    # A faire:
    true if joueur.en_vie == false || monde.ennemis_en_vie.size == 0
